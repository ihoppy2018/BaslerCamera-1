...
// 以时间作为图片名 精确到毫秒
string myAlgorithm::GetTimeAsFileName()
{
    char filename[100] = { 0 };
    SYSTEMTIME st = { 0 };
...
// 识别一维码 自动识别模式
cv::Mat myAlgorithm::detect_bar_auto(cv::Mat input)
{
    if (input.empty())
    {
    ...
    for (int i = 0; i < barcodes.size(); i++)
    {
        ...
        for (; symbol != imageZbar.symbol_end(); ++symbol)
        {
            ...
            (message==NULL)?(qDebug() << ("message is null")):(qDebug()<<("message is nonull"));
            message->append("type " + QString().fromStdString(symbol->get_type_name()));
            message->append("number " + QString().fromStdString(symbol->get_data()));
            //emit sendMessage(QString().fromStdString(symbol->get_type_name()));
            //emit sendMessage(QString().fromStdString(symbol->get_data()));
...
// 保存结果
void myAlgorithm::saveResult(cv::Mat img, std::string path, std::string filename)
{
    // 检查目录是否存在，若不存在则新建
    QDir dir;
    ...
    if (!dir.exists(qpath))
    {
    bool res = dir.mkpath(qpath);
    qDebug() << "new dir created:" << qpath << res;
    }
    bool isSave = cv::imwrite(path + filename, img);
    if(!isSave)
    {
